    ...
    // Check for assassin timeout
    {
      ...
      if (assassin_id != -1) {
        ...
        if (duration >= 60) {
          ...
          Color original_color = original_assassin_color;

          if (game.players.count(assassin_id)) {
            Color old_color = game.players.at(assassin_id).color;
            game.players.at(assassin_id).color = original_color;
            username = game.players.at(assassin_id).username;
            player_exists = true;

            std::cout << "Server: Player " << assassin_id << " color changed from INVISIBLE back to " << color_to_string(original_color) << " (assassin event ended)" << std::endl;
          } else {
